# h1 Freedom of Action, Control, and Risk Mitigation

## a) ISMS-scope

### a1) Mitä ISMS-scopeeni kuuluu?

ISMS-scopeen kuuluu oma kotiverkko ja IT-ympäristö, jota käytän opiskeluun ja kurssin harjoitusten tekemiseen

**Infrastruktuuri:**
- Reititin ja Wi-Fi-verkko
- Reitittimen palomuuri

**Laitteet:**
- Windows-läppäri, jossa on Debian Linux -virtuaalikone
- Windows 11 pöytäkone, jossa on Debian Linux -virtuaalikone
- MacBook - air m4, joka toimii minulla ensisijaisena opiskelukoneena silloin kun ei tarvitse linuxia
- Puhelin - OxygenOS 14, jota käytän MFA-autentikointiin

**Informaatio ja data:**
- Moodlessa olevat kurssimateriaalit
- Teron kurssisivuilla olevat tehtävänannot ja kurssimateriaalit
- Oma GitHub-repositorio
- OneDrive-varmuuskopiointi
- Omat muistiinpanot
- Autentikointitiedot

Debian-virtuaalikoneet kuuluvat scopeen, koska niitä käytetään kurssin harjoituksiin joihin tarvitsee linuxia. Mäkkiä käytän kaikkeen missä en tarvitse linuxia. Puhelinta käytän MFA-autentikointiin.

### a2) Mitä ISMS-scopeeni ei kuulu ja miksi?

Seuraavat asiat eivät kuulu ISMS-scopeeni:

- **Smart TV**, koska sitä ei käytetä opiskeluun tai kurssin harjoituksiin
- **Henkilökohtaiset tiedostot ja tunnukset**, jotka eivät liity kurssitöiden tekemiseen
- **ISP:n infrastruktuuri oman reitittimeni ulkopuolella**, koska se ei ole omassa hallinnassani
- 
Nämä on rajattu scopesta ulos, koska ne eivät ole kurssin kannalta tarpeellisia tai niitä ei ole mahdollista hallita itse.

### a3) Rajat ja rajapinnat

Ympäristöni tärkeimmät rajat ja rajapinnat ovat:

- **Reititin / Wi-Fi:** muodostaa rajan oman kotiverkon ja internetin välillä. Reitittimen palomuuri suojaa kotiverkkoa ulkoiselta liikenteeltä
- **ISP (Telia):** tarjoaa nettiyhteyden, mutta Telian oma infrastruktuuri on oman ISMS-scopen ulkopuolella
- **GitHub:** ulkoinen pilvipalvelu, jossa on kurssirepositiorioni
- **Moodle:** Käytän kurssimateriaaleihin ja tehtäviin
- **OneDrive:** Käytän opiskeluun liittyvien tiedostojen tallentamiseen ja varmuuskopiointiin
- **Teron kurssisivut:** ulkoinen sivu, jossa on tehtävänannot
- **Debian-virtuaalikoneet:** muodostavat erillisen linux-ympäristön fyysisten tietokoneiden sisällä
- **MFA:** puhelin toimii rajapintana ulkoisiin autentikointipalveluihin silloin, kun sitä käytetään MFA-tunnistautumiseen

Kotiverkon ja Internetin välinen raja kulkee käytännössä reitittimen kautta. Ulkoiset pilvipalvelut ja muut Internetissä sijaitsevat palvelut ovat ympäristöni ulkopuolella, mutta niihin liittyvät omat käyttäjätilini ja opiskeluun liittyvä data kuuluvat scopeen siltä osin kuin niitä käytetään opiskeluun

## Todisteet

- **Reititin ja Wi-Fi:** Kuvakaappaus reitittimen asetussivulta, josta voidaan todentaa verkon ja Wi-Fi:n asetukset
  
  ![H1 tehtävä](https://github.com/Ralchuu/sovellustenhakkerointi/blob/main/h1teht%C3%A4v%C3%A4.png?raw=true)
  
  ![H1 tehtävä 2](https://github.com/Ralchuu/sovellustenhakkerointi/blob/main/h1teht%C3%A4v%C3%A42.png?raw=true)

- **Laitteet:** Laiteluettelo, josta näkyvät ISMS-scopeen kuuluvat tietokoneet ja puhelin
  
  ![H1 tehtävä 6](https://github.com/Ralchuu/sovellustenhakkerointi/blob/main/h1teht%C3%A4v%C3%A46.png?raw=true)

- **Virtuaalikoneet:** Kuvakaappaus virtuaalikoneohjelmistosta, josta näkyvät kurssilla käytettävät Debian-virtuaalikoneet
  
  ![H1 tehtävä 3](https://github.com/Ralchuu/sovellustenhakkerointi/blob/main/h1teht%C3%A4v%C3%A43.png?raw=true)
  
  ![H1 tehtävä 4](https://github.com/Ralchuu/sovellustenhakkerointi/blob/main/h1teht%C3%A4v%C3%A44.png?raw=true)

- **GitHub:** Linkki kurssitehtävissä käytettyyn GitHub-repositorioon
  
 [GitHub-repositorio](https://github.com/Ralchuu/sovellustenhakkerointi/tree/main)

- **Moodle:** Kuvakaappaus Moodle-kurssista
  
  ![H1 tehtävä 7](https://github.com/Ralchuu/sovellustenhakkerointi/blob/main/h1teht%C3%A4v%C3%A47.png?raw=true)

- **OneDrive:** Kuvakaappaus OneDriven käytöstä
  
 ![H1 tehtävä 5](https://github.com/Ralchuu/sovellustenhakkerointi/blob/main/h1teht%C3%A4v%C3%A45.png?raw=true)

- **MFA:** Kuvakaappaus käyttäjätilin turvallisuusasetuksista, josta näkyy MFA:n olevan käytössä


## Verkko- ja rajapintakaavio

```mermaid
flowchart LR
    subgraph IN["Home Network - In Scope"]
        Laptop["Windows-läppäri<br/>+ Debian VM"]
        Desktop["Pöytäkone<br/>+ Debian VM"]
        Mac["MacBook"]
        Phone["Puhelin / MFA"]
        Router["Reititin / Wi-Fi / Palomuuri"]

        Laptop --> Router
        Desktop --> Router
        Mac --> Router
        Phone --> Router
    end

    Router --> Internet["Internet"]

    subgraph OUT[" "]
        GitHub["GitHub"]
        Moodle["Moodle"]
        OneDrive["OneDrive"]
        Tero["Teron kurssisivut"]
    end

    Internet --> GitHub
    Internet --> Moodle
    Internet --> OneDrive
    Internet --> Tero

    subgraph EXT["Home Network - Out of Scope"]
        SmartTV["Smart TV"]
    end
```

## b) Linking the Assignment to the Standard

| Interested Party | Need or Requirement | ISO 27001 Reference (Requirement Area) | How Compliance Is Demonstrated (Evidence) |
|---|---|---|---|
| **Opiskelija (minä)** | Kurssitehtävien ja harjoitusten tulee olla käytettävissä ja omien tietojen sekä repositorioiden tulee säilyä turvassa. | **Context / Leadership / Planning** | Laitteiden ja virtuaalikoneiden dokumentointi, varmuuskopiot sekä GitHub- ja OneDrive-tiedostojen säilytys. |
| **Haaga-Helia / kurssin järjestäjä** | Kurssiharjoitukset tehdään luvallisesti ja turvallisesti. Harjoitukset eivät saa aiheuttaa haittaa muille järjestelmille tai sisältää luvatonta tietojen käsittelyä. | **Context / Operation** | Kurssin scope- ja rajausdokumentaatio, harjoitusten dokumentointi sekä kurssin sääntöjen ja annettujen ohjeiden noudattaminen. |
| **Pilvipalveluiden tarjoajat (GitHub, Microsoft)** | Tilien ja opiskeluun liittyvien tietojen tulee olla suojattuja. Käyttöehtoja ja turvallisuusvaatimuksia tulee noudattaa. | **Support / Operation** | MFA:n käyttöönotto, tilien turvallisuusasetukset sekä GitHub- ja OneDrive-käyttöoikeuksien tarkistaminen. |

## Lähteet

SFS, 04.08.2023.SFS-EN ISO/IEC 27001:2023. Tietoturvallisuus, kyberturvallisuus ja tietosuoja. Tietoturvallisuuden hallintajärjestelmät. Vaatimukset. Helsinki. Luettu 25.08.2026.
